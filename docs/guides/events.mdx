...
it is no longer hidden from assistive tech. Cell presses and drag-to-create are
unreachable underneath such a band. Define the component outside your render so
it keeps its identity (on the web the args type is
