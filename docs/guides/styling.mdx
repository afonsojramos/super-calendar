...
- **Slot names differ where the structure does.** Native adds text slots
  ...
  through `dayColumn`. It omits dom-only slots (`dayColumn`, `events`, `chipButton`,
  `chip`, `eventBox`).
- **Event chips and cards are `renderEvent`'s job.** To restyle them with Tailwind,
  ...
  components.
- **Animated slots.** `businessHours`, `weekendShade`, `daySeparator`, `event`,
  `nowIndicator`, and `createGhost` render as Reanimated
...
slots, `hourGutter`, `hourLabel`, `gridLines`, `businessHours`, `backgroundEvent`,
`weekendShade`, `daySeparator`,
`event`, `nowIndicator`, and `createGhost`. The day-header row is a fixed 56px
tall so the columns, the all-day band, and the hour column line up; keep custom
`columnHeader` content within that height.
...
| `allDayLabel`         | The "all-day" gutter label (with `showAllDayLabel`) |
