...
<Note>
  This is a **web renderer** (`@super-calendar/dom`) view — a scheduling board with optional
  drag-to-move and edge-resize.
</Note>
...
axis, or drag its right edge to resize. It's called with the proposed new
start/end; return `false` to reject the drop (the bar snaps back). Movement snaps
to `dragStepMinutes` (default 15). A bar being dragged carries `data-dragging` for
styling.
