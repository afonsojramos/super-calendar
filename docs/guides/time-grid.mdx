...
lane on top. On React Native the day header and the all-day lane page with the
columns while the hour column stays fixed on the left, so the whole grid moves as
one when you swipe. `week` pages by the calendar week (set `weekStartsOn`: 0 = Sunday,
...
- The hour column is a single column beside the day columns, in the same
  scroll view, so it scrolls and zooms with them and holds still while you
  swipe between pages. Style it through the `hourGutter` [slot](/guides/styling).
...
On React Native, `refreshControl` takes a `RefreshControl` element and wraps the
grid's vertical scroll view, so a pull from the top can reload the events. The
pull is only reachable with the grid scrolled to its very top, which the default
`scrollOffsetMinutes` starts you away from. On Android that same layout also
catches a page swipe with a downward component while sitting at the top, so it
refreshes instead of paging.

## The all-day lane

All-day events (flagged `allDay`, or spanning midnight-to-midnight) leave the
timed columns and stack in a lane above the grid. On React Native the lane pages
with the day columns and stays pinned above the hours while you scroll. Its
height follows the week under it as you swipe, and it collapses to nothing for a
week with no all-day events. The "all-day" text label is off by default so the
lane can come and go without a stranded label; set `showAllDayLabel` to bring it
back. Turn the lane off entirely with `showAllDayEventCell={false}`.
...
- `renderTimeGridHeader={(days: Date[]) => ReactNode}` **replaces** the built-in
  day-header row with a single fixed header above the grid (it does not page).
  The week-number gutter and the tappable weekday/date buttons disappear, and
  your element renders full-width in their place. `days` is the visible page's
  dates in column order. The all-day lane and the grid itself stay. To line your
  columns up with the grid's, leave a leading spacer as wide as the hour gutter
  (`hourColumnWidth`, default 56, or 0 with `hideHours`). The built-in header,
  by contrast, pages with the columns.
- `headerComponent={<YourToolbar />}` **inserts** an element, unchanged, above
  the grid. It takes no arguments and replaces nothing, and it doesn't scroll
  with the hours. The built-in day header now pages with the columns, so
  `headerComponent` sits above that paged weekday/date row. Use it for a filter
  row, an avatar strip, or your own toolbar.
