...
with the day columns and stays pinned above the hours while you scroll. It is
always shown, at least one row tall, and its height follows the week under it as
you swipe. The "all-day" text label is off by default so an empty row reads as
neutral space; set `showAllDayLabel` to bring it back. Turn the lane off entirely
with `showAllDayEventCell={false}`.
