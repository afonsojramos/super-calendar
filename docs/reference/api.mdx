...
The native package declares a React Native peer range of `>=0.74 <2`, which
includes stable 1.x versions. Use versions of Reanimated, Worklets, and Gesture
Handler that are compatible with your React Native version.
...
| `renderBackgroundEvent`    | `RenderEvent`                      | Custom, pressable background bands.      |
...
| `showAllDayEventCell`  | `boolean`                                  | All-day lane (default on; grows with all-day events, none when empty).          |
| `showAllDayLabel`      | `boolean`                                  | Time grid: show the "all-day" text label (default off).                         |
| `refreshControl`       | `ScrollViewProps["refreshControl"]`        | Pull-to-refresh control for schedule, time-grid and year modes (React Native).  |
